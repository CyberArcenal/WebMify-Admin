// src/pages/comments/components/CommentActionsDropdown.tsx
...
const CommentActionsDropdown: React.FC<CommentActionsDropdownProps> = ({
  comment,
  onView,
  onEdit,
  onDelete,
  ...
  return (
    <div className="comment-actions-dropdown-container" ref={dropdownRef}>
      <button
        ref={buttonRef}
        onClick={(e) => {
          e.stopPropagation();
      ...
      {isOpen && (
        ...
        >
          <div className="py-1">
            {/* View Details */}
            <button
              onClick={() => handleAction(() => onView(comment))}
            ...
            {comment.approved ? (
              <button
                onClick={() => handleAction(() => onToggleApproved?.(comment))}
                className="w-full flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100"
              >
