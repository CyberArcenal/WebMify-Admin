// src/pages/skills/components/SkillActionsDropdown.tsx
...
const SkillActionsDropdown: React.FC<SkillActionsDropdownProps> = ({
  skill,
  onView,
  onEdit,
  onDelete,
  ...
  return (
    <div className="skill-actions-dropdown-container" ref={dropdownRef}>
      <button
        ref={buttonRef}
        onClick={(e) => {
          e.stopPropagation();
      ...
      {isOpen && (
        ...
        >
          <div className="py-1">
            {/* View Details */}
            <button
              onClick={() => handleAction(() => onView(skill))}
            ...
            {/* Edit */}
            <button
              onClick={() => handleAction(() => onEdit(skill))}
              className="w-full flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100"
            >
            ...
            {skill.featured ? (
              <button
                onClick={() => handleAction(() => onToggleFeatured?.(skill))}
                className="w-full flex items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100"
              >
